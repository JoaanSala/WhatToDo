...
object CreditCardUtils {
    ...
    fun getCardType(cardNumber: String): Int {
        if (cardNumber.substring(0, 1) == VISA_PREFIX) return VISA else if (MASTERCARD_PREFIX.contains(cardNumber.substring(0, 2) + ",")) return MASTERCARD else if (AMEX_PREFIX.contains(cardNumber.substring(0, 2) + ",")) return AMEX else if (cardNumber.substring(0, 4) == DISCOVER_PREFIX) return DISCOVER
        return NONE
    }

    @JvmStatic
    fun isValid(cardNumber: String): Boolean {
        if (!TextUtils.isEmpty(cardNumber) && cardNumber.length >= 4) if (getCardType(cardNumber) == VISA && (cardNumber.length == 13 || cardNumber.length == 16)) return true else if (getCardType(cardNumber) == MASTERCARD && cardNumber.length == 16) return true else if (getCardType(cardNumber) == AMEX && cardNumber.length == 15) return true else if (getCardType(cardNumber) == DISCOVER && cardNumber.length == 16) return true
        return false
    }
